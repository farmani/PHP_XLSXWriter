...
class XLSXWriter
{
    ...
    protected function initializeSheet($sheet_name, $columns=[], $hidden=[], $locked=[])
    {
        ...
        if(!empty($columns)) {
            ...
            foreach ($columns as $key=>$column) {
                $i++;
                if($hidden[$key])
                    $sheet->file_writer->write(    '<col collapsed="true" locked="true" hidden="true" min="'.$i.'" max="'.$i.'" style="0" width="11.5"/>');
                else
                    $sheet->file_writer->write(    '<col collapsed="false" hidden="false" min="'.$i.'" max="'.$i.'" style="0" width="11.5"/>');
            }
        } else
    ...
    public function writeSheetHeader($sheet_name, array $header_types, $suppress_row = false, $hidden=[], $locked=[])
    {
        if (empty($sheet_name) || empty($header_types) || !empty($this->sheets[$sheet_name]))
            return;
        self::initializeSheet($sheet_name, $header_types, $hidden, $locked);
        $sheet = &$this->sheets[$sheet_name];
        $sheet->columns = array();
    ...
    public function writeSheetRow($sheet_name, array $row, $hidden=[], $locked=[])
    {
        if (empty($sheet_name) || empty($row))
            return;
        self::initializeSheet($sheet_name, $row, $hidden, $locked);
        $sheet = &$this->sheets[$sheet_name];
